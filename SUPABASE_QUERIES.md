# HousingWorld: Supabase Queries & Database Setup

This file contains the complete SQL schema, Row-Level Security (RLS) policies, Storage bucket configurations, and seed data required to set up the database for the HousingWorld application.

---

## 1. Users Table Schema & RLS Policies

This table holds synchronization data matching users created via **Clerk Authentication**.
...
## 2. Properties Table Schema & RLS Policies

Holds all property listings.
...
create table properties (
  ...
  latitude float,
  longitude float,
  images text[] default '{}', -- Array of Supabase Storage/CDN image URLs
...
-- Public read policy (anyone can browse properties)
...
## 3. Saved Properties (Bookmarks) Schema & RLS Policies

Handles user favorites/bookmarks.

```sql
-- Create Saved Properties Junction Table
create table saved_properties (
  id uuid default gen_random_uuid() primary key,
  user_clerk_id text not null references users(clerk_id) on delete cascade,
  property_id uuid not null references properties(id) on delete cascade,
  created_at timestamp with time zone default now(),
  unique(user_clerk_id, property_id) -- Prevents duplicate saves for the same property
);
...
## 4. Storage Bucket Setup (Property Images)

Run this to create the storage bucket for hosting property listing images and set public read policies.
...
-- Allow anyone to read images (they are public listings)
...
## 5. Admin Authorization Policies

These policies ensure only admins (where `is_admin = true` in the `users` table) can perform write/edit/delete actions on properties and storage.

```sql
-- Admin write/update/delete policies for Properties
...
-- Admin upload policy for Storage Images
...
## 6. Seed Data (Dummy Properties)

Run this query to populate the database with featured and recommended properties across major Indian cities.
